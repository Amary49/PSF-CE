...
`READY_FOR_PUBLIC_GITHUB_UPLOAD`。本次公开 GitHub Release 为 `v1.2.0`；内部审计包 V10 已与英文稿 V9.3、26 个实际引用键及最终 ZIP/PDF 哈希对齐。V5/V9.3/V10 是内部溯源标识，不是 GitHub 上传次数；既有算法、预测、分数、参数、图表和正式数据集均未改变。
...
- 若以后希望公开 YACHT 端到端运行能力，需另行确认 `psfce_v4_yacht_walk` 的代码来源与可再分发性；当前版本不包含该 helper。
...
运行 `python scripts/validate_release.py`，并确认 `python -m pytest -q` 通过。V10 的归档 ZIP 必须按上述规则重新生成 manifest 和 SHA-256；不得把私有审计、旧版 ZIP 或本机路径映射一并上传。
