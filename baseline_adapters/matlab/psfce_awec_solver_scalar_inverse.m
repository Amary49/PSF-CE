function [C,Z] = psfce_awec_solver_scalar_inverse(CA,A,lambda,gamma,K)
% Author solver_AWTP with one exact algebraic simplification:
% inv(s*I)*B is evaluated as (1/s)*B. All iterative operations are unchanged.
n = size(CA,1);
t = 0;
e = 1e-2;
max_iter = 100;
...
while t < max_iter
    ...
    C = pre_scale * (mu * P1 + mu * P2 + G*G' + 2*alphaA);
    D = diag(sum(C));
    E = mu * (CA - C) + Y1;
    E = E /(lambda + mu);
    F = C + Y2 / mu;
